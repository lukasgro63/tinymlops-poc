...
export const getDriftStatisticsByDevice = async (
  days: number = 30,
  drift_type?: string,
  limit: number = 10
  ...
  const response = await axios.get<DeviceStatusData[]>(
    `${API_BASE_URL}/drift-dashboard/statistics/by-device?${params.toString()}`
  );
  return response.data;
};

export const getDriftTimelineByDevice = async (
  period: 'day' | 'week' | 'month' = 'week',
  drift_type?: string,
  limit: number = 10
): Promise<DriftTimelineData[]> => {
  const params = new URLSearchParams();
  ...
  const response = await axios.get<DriftTimelineData[]>(
    `${API_BASE_URL}/drift-dashboard/timeline/by-device?${params.toString()}`
  );
  return response.data;
};
