...
def perform_statistical_tests(scenarios_data: dict) -> pd.DataFrame:
    ...
    for scenario, df in scenarios_data.items():
        ...
        metrics[scenario] = {
            ...
            "latency": df[inference_col].values if inference_col else np.array([]),
    ...
    for comparison in [("scenario1", "scenario0"), ("scenario2_1", "scenario0")]:
        ...
        for metric_name, metric_key in [("CPU", "cpu"), ("Memory", "memory"), ("Latency", "latency")]:
            if len(metrics[s1][metric_key]) > 0 and len(metrics[s0][metric_key]) > 0:
                ...
                min_len = min(len(metrics[s1][metric_key]), len(metrics[s0][metric_key]))
                data1 = metrics[s1][metric_key][:min_len]
                data0 = metrics[s0][metric_key][:min_len]
                
                # Wilcoxon test
                stat, p_value = stats.wilcoxon(data1, data0)
                ...
                print(f"  {metric_name}: p={p_value:.4f} ({significant})")
    ...
    for comparison in [("scenario1", "scenario0"), ("scenario2_1", "scenario0")]:
        ...
        if len(metrics[s1]["cpu"]) > 0 and len(metrics[s0]["cpu"]) > 0:
            min_len = min(len(metrics[s1]["cpu"]), len(metrics[s0]["cpu"]))
            diff_cpu = metrics[s1]["cpu"][:min_len] - metrics[s0]["cpu"][:min_len]
            ...
            print(f"  CPU (±{sesoi_cpu}pp): p={p_tost:.4f} ({equivalent})")
        
        # Latency equivalence
        if len(metrics[s1]["latency"]) > 0 and len(metrics[s0]["latency"]) > 0:
            min_len = min(len(metrics[s1]["latency"]), len(metrics[s0]["latency"]))
            diff_latency = metrics[s1]["latency"][:min_len] - metrics[s0]["latency"][:min_len]
            ...
            print(f"  Latency (±{sesoi_latency}ms): p={p_tost:.4f} ({equivalent})")
    
    # 5. Memory trend test (Mann-Kendall)
    print("\n=== 5. Memory Trend Tests (Mann-Kendall) ===")
    for scenario in ["scenario0", "scenario1", "scenario2_1"]:
        if len(metrics[scenario]["memory"]) > 10:  # Need sufficient data points
            trend_result = mk.original_test(metrics[scenario]["memory"])
            ...
            print(f"{SCENARIO_NAMES[scenario]}: p={trend_result.p:.4f} ({trend})")
...
def plot_timing_breakdown(scenarios_data: dict, save_path: Path = None):
    ...
    # Define grayscale colors for components
    colors = {
        "Feature Extraction": "#000000",
        "KNN Inference": "#404040",
        "Drift Check": "#808080",
        "Other Overhead": "#B0B0B0",
        "Inference": "#606060"
    }
